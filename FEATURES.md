...
- The version-1 vapid.party compatibility contract is explicitly scoped to app ID `converge.cv`. It uses standard Web Push subscription data and makes no FCM-, APNs-, or browser-vendor assumption. XMTP topic/HMAC registration is the generic logical layer; other apps or delivery adapters require their own app-scoped authenticated contract and are not silently added to Converge's public route.
- The vapid.party contract supports public VAPID lookup, logical per-inbox registration on a shared endpoint, logical deletion, authenticated ciphertext-envelope delivery, and a coarse public health response. Its deployed Cloudflare-only runtime uses a Worker, D1 registration bridge, delivery Queue, and singleton Container XMTP listener. Registration success alone never changes delivery readiness to ready.
...
- Notification setup results remain visible inline in Settings and Debug instead of relying on transient browser alerts; Debug also prevents overlapping enable attempts. Diagnostics separately report the browser endpoint, relay API reachability, and end-to-end XMTP delivery readiness. A generic healthy Worker response is shown as unknown, not ready.
