...
- If the saved installation and opened database still differ, Settings shows a typed repair state with the live installation count instead of repeatedly returning a generic reconnect error. No ledger mutation happens during detection or ordinary retry. The explicit **Repair This Browser** action persists the same inspected candidate before any mutation, rechecks the signer/inbox/capacity, and registers that candidate at most once. When the current signer is recovery authority it may revoke only the exact saved unavailable installation; otherwise it uses one free slot and preserves the old ID for later exact cleanup. It never chooses an arbitrary or merely old device, deletes messages, or clears the XMTP database.
- A repair interrupted after revoke or registration resumes from the persisted candidate and settles against fresh local plus ledger state rather than creating another candidate. At 10/10 it stops before registration unless exact saved-installation revocation is authorized. A locally registered database that remains absent from repeated ledger verification is left untouched for manual recovery; generic lock, quota, and open failures never trigger deletion.
