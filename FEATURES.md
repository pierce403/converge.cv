...
- Brave-specific provider errors are detected through `navigator.brave` and explain that **Use Google services for push messaging** must be enabled followed by a full browser quit/relaunch. Notification permission can remain granted, and already visible app, native, or extension notifications do not prove that Brave will accept a new origin subscription.
