...
- Composer activity sends Convos-compatible `convos.org/typing_indicator:1.0` messages with `shouldPush:false`. Inbound Convos indicators and the agent-facing `cthuwu.app/typing:1.0` control render as one transient animated three-dot bubble, honor bounded expiry, and are never persisted as chat history. Older placeholder rows named `Typing`/`Thinking` are removed when the conversation loads so previews and history repair themselves.
