...
- Reload and restore paths never infer registration from `inboxId` presence and never retry `register()` blindly. They pin the persisted installation ID and first probe only the two deterministic database paths already used by Converge. An alternate database is adopted automatically only when both local registration state and fresh inbox state prove that exact installation is already registered.
- If the saved installation and opened database still differ, Settings shows a typed repair state with the live installation count instead of repeatedly returning a generic reconnect error. No ledger mutation happens during detection or ordinary retry. The explicit **Repair This Browser** action opens the local database once, persists the installation ID from that live attempt before any mutation, and registers it on the same open client without reopening. When capacity remains, the replacement is verified before exact cleanup of the saved unavailable installation; only at 10/10 can recovery-authorized exact cleanup happen first. A non-recovery signer uses one free slot and preserves the old ID for later cleanup. Repair never chooses an arbitrary or merely old device, deletes messages, or clears the XMTP database.
- Closing an XMTP worker before registration can cause the Browser SDK to generate a different prospective installation ID the next time that database opens. A repair interrupted after revoke or registration therefore resumes by settling the persisted journal against fresh local and ledger state; it cannot promise to recover the same unregistered ID after a crash or reload. A new live attempt may persist the ID it opens, but registers on that same client without reopening and retains the exact prior installation as the only cleanup target. At 10/10 it stops before registration unless exact saved-installation revocation is authorized. A locally registered database absent from repeated ledger verification, or a ledger-visible partial registration whose local key was lost with the worker, is left untouched for manual recovery; generic lock, quota, and open failures never trigger deletion.
