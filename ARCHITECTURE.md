...
**Repair This Browser** is a distinct, confirmed lifecycle operation. It holds an inbox-scoped `navigator.locks` lease when available, reopens the exact classified candidate with auto-registration disabled, refetches the expected inbox and signer authority, and persists `{installationId: candidate, staleInstallationId: prior, installationRepairPending: true, xmtpDbPathMode}` before the first ledger mutation. If the signer is the inbox recovery identity, repair may revoke only the exact prior installation bytes and settles a thrown response through fresh state. A non-recovery account key never revokes another installation; it may register the persisted candidate only when capacity remains. Capacity is checked again immediately before registration, and `existing-inbox` registration then verifies local readiness plus exact ledger membership. A new replacement requests encrypted device history.

Crashes resume the same candidate: a prior ID is retained for exact cleanup, registration responses are settled before retry, and no second candidate is generated. The operation never chooses the oldest installation, clears Dexie, or deletes OPFS. A locally registered candidate that is absent from fresh ledger state remains fail-closed because the SDK exposes no integrity check that distinguishes corruption from network, lock, or quota failures.
