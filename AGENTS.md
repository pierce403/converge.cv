...
- Settled reconnect probes both deterministic XMTP database paths without creating an alternate-path file, and automatically repairs metadata only for an already-registered, ledger-visible installation. Other installation mismatches surface an explicit Settings repair. A live repair opens the database once, persists that attempt's candidate before mutation, registers it without reopening, may revoke only the exact unavailable saved ID when the signer is recovery authority, uses a free slot otherwise, rechecks 10/10 capacity, requests history, and never clears messages or OPFS. With capacity available it verifies the replacement before attempting exact cleanup; only at 10/10 may exact cleanup precede registration. Because Browser SDK 6.1.2 may generate a different prospective installation ID after an unregistered worker closes, interrupted repair resumes from fresh journal and ledger state without promising the same unregistered ID; locally-ready/ledger-absent and ledger-visible/locally-unready states remain fail-closed rather than rotating through installations.
