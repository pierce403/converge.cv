...
- Settled reconnect probes both deterministic XMTP database paths without creating an alternate-path file, and automatically repairs metadata only for an already-registered, ledger-visible installation. Other installation mismatches surface an explicit Settings repair. That repair persists one exact candidate before mutation, may revoke only the exact unavailable saved ID when the signer is recovery authority, uses a free slot otherwise, rechecks 10/10 capacity, requests history, and never clears messages or OPFS. Interrupted repair must resume the same candidate; a locally registered but ledger-absent database remains fail-closed.
...
- Current Vitest status (2026-08-12): `pnpm test --run` passes (89 files, 590 tests).
