...
- Current Vitest status (2026-07-14): `pnpm test --run` passes (81 files, 531 tests).
...
- The version-1 Converge registration wire remains compatible with vapid.party, but the client models it as an app-scoped XMTP alert registration with standard Web Push as the current provider-neutral delivery adapter.
...
- CI-equivalent verification passes: typecheck, zero-warning lint, 81 Vitest files (531 tests), and the production build.
