...
} from '@/lib/message-retention-policy';
...
import { runInstallationRepairSession } from './installation-repair';
import {
...
function decodedMessageExpiryMs(message: unknown): number | undefined {
  ...
  return typeof expiresAtNs === 'bigint' ? Number(expiresAtNs / 1_000_000n) : undefined;
}
...
function getXmtpSdkLogLevel(): LogLevel {
  ...
  // worker bridge `console.log` output ("client received event data"). Default to Off.
  try {
    const raw =
      typeof window !== 'undefined'
        ? window.localStorage.getItem(XMTP_SDK_LOG_LEVEL_STORAGE_KEY)
        : null;
...
interface RetainedInstallationRepairClient {
...
export class XmtpClient {
  ...
  private retainedInstallationRepairClient: RetainedInstallationRepairClient | null = null;
  ...
  private dmByInboxCache = new KeyedAsyncCache<unknown | null>({
    ...
    onHit: () => {
      this.cacheCounters.dmByInbox.hit += 1;
    },
    onMiss: () => {
      this.cacheCounters.dmByInbox.miss += 1;
    },
  });
  private inboxStatesCache = new KeyedAsyncCache<InboxState | null>({
    ttlMs: 6 * 60 * 60 * 1000,
    isNegative: (value) => value == null,
    onHit: () => {
      this.cacheCounters.inboxState.hit += 1;
    },
    onMiss: () => {
  ...
  private stopBackgroundDiscoveryLoop(): void {
    this.backgroundDiscoveryGeneration += 1;
  ...
  private clearSyncStatusResetTimer(): void {
    if (this.syncStatusResetTimer) {
      clearTimeout(this.syncStatusResetTimer);
  ...
  private async getDmByInboxIdCached(inboxId: string): Promise<unknown | null> {
    ...
    const key = this.normalizeInboxIdKey(inboxId);
    if (!key) return null;

    return await this.dmByInboxCache.get(key, async () => {
      try {
        const dm = await this.client!.conversations.getDmByInboxId(key);
        return dm ?? null;
      } catch {
        return null;
      }
  ...
  private async fetchInboxStateCached(inboxId: string): Promise<InboxState | null> {
    const key = this.normalizeInboxIdKey(inboxId);
    if (!key) return null;

    return await this.inboxStatesCache.get(key, async () => {
      try {
        if (this.client) {
          const latestStates = await this.retryWithBackoff(
            'preferences.fetchInboxStates',
            () => this.client!.preferences.fetchInboxStates([key]),
          );
          return (latestStates as InboxState[] | undefined)?.[0] ?? null;
        }

        const states = await this.retryWithBackoff(
          'client.fetchInboxStates',
          () => Client.fetchInboxStates([key], 'production'),
        );
        return (states as Array<InboxState | null | undefined> | undefined)?.[0] ?? null;
      } catch {
        return this.inboxStatesCache.peek(key) ?? null;
      }
    });
  }
  ...
  private static sanitizeConvosProfileImageUrl(value?: string): string | undefined {
    const sanitized = XmtpClient.sanitizeProfileAvatarUrl(value);
    if (!sanitized) return undefined;
    // Convos stores image references as URLs in appData. Data URLs are too large for metadata and not interoperable.
    if (/^data:/i.test(sanitized)) return undefined;
    return sanitized;
  }
  ...
  async sendReply(
    ...
    if (!this.client) {
      throw new Error('XMTP is not connected. Reconnect before sending this reply.');
    }
  ...
  getInboxProfileLocal(inboxId: string): InboxProfile {
    return this.buildInboxProfileLocal(inboxId);
  }

  async refreshInboxProfile(
    inboxId: string,
    opts?: {
      allowRecovery?: boolean;
    }
  ): Promise<InboxProfile> {
    return await this.fetchInboxProfile(inboxId, { ...opts, mode: 'network' });
  }

  private buildInboxProfileLocal(inboxId: string): InboxProfile {
    const raw = String(inboxId ?? '').trim();
    ...
    const looksLikeInboxId = (value: string): boolean => {
      ...
      if (v.length < 24) return false; // too short to be a real inbox id
      return /^[0-9a-f]+$/i.test(v);
    };

    const contactStore = useContactStore.getState();
    const contact =
      contactStore.getContactByInboxId(normalized) ??
      contactStore.getContactByAddress(normalized);

    if (contact) {
      const addresses = (contact.addresses ?? []).filter(Boolean).map((addr) => addr.toLowerCase());
      ...
      return {
        inboxId: contact.inboxId.toLowerCase(),
        displayName: contact.preferredName ?? contact.name,
        avatarUrl: contact.preferredAvatar ?? contact.avatar,
        primaryAddress: contact.primaryAddress?.toLowerCase() ?? addresses[0],
        addresses,
        identities,
      };
    }

    // If the value doesn't look like an inbox id, return a minimal profile.
    if (!looksLikeInboxId(normalized)) {
      const isAddressLikeInput = normalized.startsWith('0x');
      return {
        inboxId: normalized,
        displayName: isAddressLikeInput ? undefined : raw || undefined,
        avatarUrl: undefined,
  ...
  async fetchInboxProfile(
    ...
    const base = this.buildInboxProfileLocal(inboxId);
    if (mode === 'local') {
      return base;
    }
    ...
    const normalizedInboxId = base.inboxId.toLowerCase();
    ...
    // Step A: Try DM profile scan from the local DB (no dm.sync).
    if (this.client) {
      try {
        const dm = await this.getDmByInboxIdCached(normalizedInboxId);
        if (dm) {
          const msgs = await (dm as unknown as { messages: (opts?: ListMessagesOptions) => Promise<unknown[]> }).messages({
          ...
          for (const m of msgs) {
            ...
            if (myInboxId && senderInboxId === myInboxId) continue;
    ...
    const needsIdentity = base.addresses.length === 0 && base.identities.length === 0;
    ...
    const handleIdentityError = async (error: unknown): Promise<InboxProfile | null> => {
      ...
      if (recovered && recovered !== normalizedInboxId) {
        return await this.fetchInboxProfile(recovered, { allowRecovery: false, mode: 'network' });
      }

      return null;
    };

    let identifiers: Identifier[] | undefined = undefined;

    // Step B: Only fetch identity info when it is missing locally.
    if (!skipIdentityApi && needsIdentity) {
      try {
        const state = await this.fetchInboxStateCached(normalizedInboxId);
        identifiers = state?.accountIdentifiers ?? undefined;
      } catch (error) {
        if (!this.isExpectedIdentityError(error)) {
    ...
    const addresses = identifiers && identifiers.length ? addressesFromIdentifiers(identifiers) : base.addresses;
    const identityRecords = identifiers && identifiers.length ? identifiers.map(toIdentityRecord) : base.identities;
    const merged: InboxProfile = {
      inboxId: normalizedInboxId,
      displayName: overrides?.displayName ?? base.displayName,
      avatarUrl: overrides?.avatarUrl ?? base.avatarUrl,
      primaryAddress: addresses[0] ?? base.primaryAddress,
      addresses,
      identities: identityRecords,
    };

    // Step C: persist any newly discovered info locally.
    if (
      (overrides?.displayName || overrides?.avatarUrl || identityRecords.length > 0 || addresses.length > 0) &&
      !normalizedInboxId.startsWith('0x')
    ) {
      try {
        await useContactStore.getState().upsertContactProfile({
          ...
          primaryAddress: merged.primaryAddress,
          addresses: merged.addresses,
          identities: merged.identities,
          source: 'inbox',
        });
      } catch (e) {
        console.warn('[XMTP] fetchInboxProfile: failed to persist contact profile (non-fatal)', e);
      }
    }
  ...
  private async buildGroupDetails(conversationId: string, group: Awaited<ReturnType<typeof this.getGroupConversation>>): Promise<GroupDetails> {
    ...
    const legacyMetadata = parseConvosGroupMetadata(rawDescription);
    const rawAppData = typeof safeGroup.appData === 'string' ? safeGroup.appData : '';
    const appMetadata = await parseConvosGroupAppData(rawAppData);
    const profileByInbox = new Map<string, ConvosGroupProfile>();
    for (const profile of appMetadata.profiles ?? []) {
      const normalized = profile.inboxId.trim().replace(/^0x/i, '').toLowerCase();
    ...
    const resolvedDescription = legacyMetadata.description ?? rawDescription ?? '';
    ...
    return {
      ...
      inviteTag: appMetadata.tag ?? legacyMetadata.tag,
  ...
  async updateGroupMetadata(
    ...
    try {
      ...
      if (updates.description !== undefined && typeof group.updateDescription === 'function') {
        ...
        const legacyMetadata = parseConvosGroupMetadata(
          typeof group.description === 'string' ? group.description : '',
        );
        const appMetadata = await parseConvosGroupAppData(
          typeof group.appData === 'string' ? group.appData : '',
        );
        // Preserve legacy description-embedded tag only when appData does not already carry the tag.
        if (!appMetadata.tag && legacyMetadata.tag) {
          descriptionToSend = encodeConvosGroupMetadata({
            description: updates.description ?? legacyMetadata.description ?? '',
            tag: legacyMetadata.tag,
            expiresAt: legacyMetadata.expiresAt,
          });
        }
  ...
  async createConvosInvite(conversationId: string): Promise<ConvosInviteResult> {
    ...
    const appMetadata = await parseConvosGroupAppData(rawAppData);
    let inviteTag = details.inviteTag ?? appMetadata.tag ?? legacyMetadata.tag;

    if (!inviteTag) {
      inviteTag = generateConvosInviteTag();

      if (typeof group.updateAppData === 'function' && (!rawAppData || appMetadata.isEncoded)) {
        const encodedAppData = await encodeConvosGroupAppData({
          ...appMetadata,
          tag: inviteTag,
        });
        await group.updateAppData(encodedAppData);
      } else if (typeof group.updateDescription === 'function') {
        const descriptionForMetadata = legacyMetadata.description ?? details.description ?? '';
      ...
      } else if (rawAppData && !appMetadata.isEncoded) {
        console.warn('[XMTP] Group appData is present but unreadable; refusing to overwrite invite metadata');
      } else {
        console.warn('[XMTP] Group metadata update unavailable; invite tag may not propagate to other clients');
      }
    }
    ...
    const invite = createConvosInvite({
      ...
      description: (legacyMetadata.description ?? details.description)?.trim() || undefined,
      conversationExpiresAt: appMetadata.expiresAt ?? legacyMetadata.expiresAt,
    });
  ...
  async processConvosInviteJoinRequest(
    ...
    try {
      const senderProfile = await this.fetchInboxProfile(senderInboxId, { mode: 'network' });
      ...
      for (const value of members) {
        ...
        if (isEthereumAddress(trimmed)) {
          ...
          if (isLikelyInboxId(trimmed)) {
            ...
            try {
              const profile = await this.fetchInboxProfile(trimmed, { mode: 'network' });
              const hasIdentity = Boolean(profile.addresses.length || profile.identities.length);
              if (!hasIdentity) {
                isValidInbox = false;
              }
  ...
  private async closeRetainedInstallationRepairClient(): Promise<void> {
    const retained = this.retainedInstallationRepairClient;
  ...
  hasRetainedInstallationRepairClient(): boolean {
    return this.retainedInstallationRepairClient !== null;
  }

  private async takeRetainedInstallationRepairClient(
    identity: XmtpIdentity,
    recovery: InstallationRecoveryDetails,
    interruptedRepairCandidateId: string | undefined,
    expectedInboxId: string,
    databasePathMode: XmtpDbPathMode
    ...
    const candidateMatchesJournal =
      installationIdsMatch(retained.installationId, recovery.localInstallationId) ||
      installationIdsMatch(retained.installationId, interruptedRepairCandidateId);
    const matches =
      retained.signerIdentity === signerIdentityKey(identity) &&
      retained.inboxId.toLowerCase() === expectedInboxId.toLowerCase() &&
      retained.databasePathMode === databasePathMode &&
      candidateMatchesJournal;
    this.retainedInstallationRepairClient = null;
    if (matches) {
      console.info(
        '[XMTP] Retrying registration with the same live browser repair candidate.'
      );
   ...
   */
  private async activateVerifiedClient(
    client: Client<unknown>,
    identity: XmtpIdentity,
    databasePathMode: XmtpDbPathMode,
    registration: ClientRegistrationResult,
    options?: Pick<ConnectOptions, 'enableHistorySync' | 'requestHistorySync'>
    ...
    const { existingInstallationCount, installationRegistered } = registration;
    ...
    logNetworkEvent({
      ...
      details: `Register step ${installationRegistered ? 'completed' : 'skipped'}; inbox ID: ${client.inboxId}`,
    });
  ...
  connect(identity: XmtpIdentity, options?: ConnectOptions): Promise<ConnectResult> {
  ...
  private async repairInstallationInternal(
    ...
    let candidate = await this.takeRetainedInstallationRepairClient(
      identity,
      recovery,
      options.interruptedRepairCandidateId,
      expectedInboxId,
      databasePathMode
    );
    ...
    try {
      ...
      repairSession = await runInstallationRepairSession(
        {
          ...
          signerIdentifier,
          expectedInboxId,
          interruptedRepairCandidateId: options.interruptedRepairCandidateId,
        ...
        },
        {
          resolveInboxId: async (identifier) =>
            await this.retryWithBackoff(
              'installation-repair:getInboxIdForIdentifier',
              () => getInboxIdForIdentifier(identifier, 'production')
            ),
      ...
      const connection = await this.activateVerifiedClient(
        candidate,
        {
          ...
          installationId: repairSession.candidateInstallationId,
          xmtpDbPathMode: databasePathMode,
        },
        databasePathMode,
        repairSession.registration,
        {
      ...
      return {
        ...connection,
        previousInstallationId: repairSession.preparation.previousInstallationId,
        previousInstallationRevoked:
          repairSession.preparation.previousInstallationRevoked,
        previousInstallationAbsent:
          repairSession.preparation.previousInstallationAbsent,
      };
    } catch (error) {
      if (
        candidateStaged &&
        ...
        this.retainedInstallationRepairClient = {
          client: candidate,
          databasePathMode,
          inboxId: candidate.inboxId,
          installationId: candidate.installationId,
          signerIdentity: signerIdentityKey(identity),
        };
        candidate = null;
      }
      throw error;
  ...
  private async connectInternal(
    ...
    if (this.hasRetainedInstallationRepairClient()) {
      throw new Error(
        'This browser has an in-progress XMTP repair candidate in memory. Use Repair This Browser to retry it, or explicitly disconnect before checking the saved installation again.'
      );
    }
    await this.closeRetainedInstallationRepairClient();
    ...
    try {
      ...
      return await this.activateVerifiedClient(
        client,
        identity,
        resolvedDatabasePathMode,
        registration,
        {
  ...
  disconnect(): Promise<void> {
    // The lifecycle queue makes back-to-back disconnects idempotent while still
    // preserving a disconnect that was requested after an intervening connect.
    return this.enqueueLifecycle(() => this.disconnectInternal());
  }
  ...
  private async stopMessageDeletionStream(): Promise<void> {
    const stream = this.messageDeletionStream;
    const task = this.messageDeletionStreamTask;
    this.messageDeletionStream = null;
    ...
    if (stream) {
      try {
        ...
        console.error('[XMTP] Error closing message deletion stream:', error);
      }
    }
    // If end() failed, closing the client is the only remaining way to release
    ...
    if (this.messageDeletionStreamTask === task) {
      this.messageDeletionStreamTask = null;
    }
  }
  ...
  syncConversations(opts?: {
    ...
    return this.enqueueLifecycle(() => this.syncConversationsInternal(opts));
  }

  private async syncConversationsInternal(opts?: {
    force?: boolean;
    minIntervalMs?: number;
    reason?: string;
    soft?: boolean;
    ...
    try {
      ...
      if (this.isUninitializedIdentityError(error) || opts?.strict) {
        console.error('[XMTP] conversations.sync failed:', error);
        throw error;
      }
      syncFailed = true;
  ...
  async syncHistory(opts?: {
    ...
    try {
        ...
        try {
          ...
          if (recentMode) {
            ...
            if (cooldownMs > 0) {
              ...
              shouldSyncConversation = false;
            }

            const now = Date.now();
            const lastSyncedAt = storedConversation?.lastSyncedAt ?? 0;
            if (lastSyncedAt > 0 && now - lastSyncedAt < 5 * 60 * 1000) {
              shouldSyncConversation = false;
            }
          }
          ...
          const perConversationOptions = {
            sentAfterNs: getRetainedHistoryStartNs(
              requestedConversationSentAfterNs,
              retentionNow,
            ),
            limit: messageLimit,
          };
      ...
      try {
        const dmSync = this.syncCounters.dmSyncCalls - countersBefore.dmSyncCalls;
        const groupSync = this.syncCounters.groupSyncCalls - countersBefore.groupSyncCalls;
        const backfilled = this.syncCounters.backfillMessages - countersBefore.backfillMessages;
        logNetworkEvent({
          direction: 'status',
          event: 'history:backfill',
          details: `mode=${mode} dm.sync=${dmSync} group.sync=${groupSync} messages=${backfilled}`,
        });
      } catch {
  ...
  runFullHistorySync(): Promise<void> {
    return this.enqueueLifecycle(() => this.runFullHistorySyncInternal());
  }

  private async runFullHistorySyncInternal(): Promise<void> {
    if (!this.client) {
      throw new Error('Client not connected');
    }
    await this.syncHistory({ mode: 'full', strict: true });
  }
  ...
  private async startMessageDeletionStream(): Promise<void> {
    ...
    const task = (async () => {
      try {
        ...
        if (this.messageDeletionStream === stream) {
          console.error('[XMTP] Message deletion stream error:', error);
        }
      }
    })();
    this.messageDeletionStreamTask = task;
    const clear = () => {
      if (this.messageDeletionStream === stream) this.messageDeletionStream = null;
      if (this.messageDeletionStreamTask === task) this.messageDeletionStreamTask = null;
    };
  ...
  private async ensureConvosGroupProfilePublished(
    ...
    const inboxId = inboxIdRaw.replace(/^0x/i, '').toLowerCase();
    if (!inboxId) return;
  ...
  async getInboxState(): Promise<InboxState> {
    ...
    if (this.shouldSkipIdentityCalls('getInboxState')) {
      throw new Error('XMTP inbox state is temporarily unavailable while identity requests cool down.');
    }
    ...
    if (this.client) {
      ...
      throw new Error('XMTP returned no inbox state for the connected client.');
    }
    ...
    try {
      ...
      throw new Error(`XMTP returned no inbox state for ${inboxId}.`);
    } catch (error) {
   ...
   */
  revokeCurrentInstallation(options: {
    expectedInboxId: string;
    expectedInstallationId?: string;
  }): Promise<{ inboxId: string; installationId: string }> {
    return this.enqueueLifecycle(() => this.revokeCurrentInstallationInternal(options));
  }

  private async revokeCurrentInstallationInternal(options: {
    expectedInboxId: string;
    expectedInstallationId?: string;
  }): Promise<{ inboxId: string; installationId: string }> {
    if (!this.client) {
    ...
    try {
      ...
      if (isEthereumAddress(peerAddressOrInboxId)) {
        ...
        if (resolvedPeerInboxId) {
          ...
          xmtpConversation = await this.client.conversations.createGroup(
            [resolvedPeerInboxId],
            createOptions,
          );
        } else {
          ...
          xmtpConversation = await this.client.conversations.createGroupWithIdentifiers(
      ...
      if (resolvedPeerInboxId && !resolvedPeerInboxId.startsWith('0x')) {
        try {
          ...
          const profile = await this.fetchInboxProfile(resolvedPeerInboxId, { mode: 'network' });
          profileDisplayName = profile.displayName;
          profileAvatar = profile.avatarUrl;
  ...
  async createGroupConversation(participantAddresses: string[]): Promise<Conversation> {
    if (!this.client) {
      throw new Error('XMTP is not connected. Reconnect before creating this group.');
    }
    ...
    try {
      ...
      if (identifiers.length) {
        groupConversation = await this.client.conversations.createGroupWithIdentifiers(
          identifiers,
          createOptions,
        );
        ...
        groupConversation = await this.client.conversations.createGroup(inboxIds, createOptions);
      }
      ...
      console.warn('[XMTP] ❌ Failed to create group conversation via XMTP:', error);
      if (error instanceof Error) {
        console.warn('[XMTP] Error details:', {
          message: error.message,
          stack: error.stack,
      ...
      logNetworkEvent({
        direction: 'status',
        event: 'conversations:create_group:failed',
        details: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
  ...
  async sendMessage(conversationId: string, content: string): Promise<XmtpMessage> {
    if (!this.client) {
      throw new Error('XMTP is not connected. Reconnect before sending this message.');
    }
    ...
    try {
      ...
      if (isDm) {
        ...
        try {
          ...
          console.warn('[XMTP] Failed to publish group profile (non-fatal):', profileError);
        }
      }
      ...
      console.warn('[XMTP] Failed to send message via XMTP:', error);
      logNetworkEvent({
        direction: 'status',
        event: 'messages:send:failed',
        details: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
