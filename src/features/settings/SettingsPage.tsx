...
export function SettingsPage() {
  ...
  const [profileRefreshSource, setProfileRefreshSource] = useState<'ens' | 'farcaster' | null>(null);
  ...
  const handleRefreshFarcasterProfile = async () => {
    if (!profileLookupAddress || profileRefreshSource) return;
    setProfileRefreshSource('farcaster');
    setProfileRefreshFeedback(null);
    try {
      const apiKey = useFarcasterStore.getState().getEffectiveNeynarApiKey();
      if (!apiKey) {
        setProfileRefreshFeedback('Configure a Neynar key below to refresh from Farcaster.');
        return;
      }
      const fid = await resolveFidFromAddress(profileLookupAddress, apiKey);
      const profile = fid ? await fetchNeynarUserProfile(fid, apiKey) : null;
      const name = pickFarcasterDisplayName(profile);
      if (!profile || !name) {
        setProfileRefreshFeedback('No Farcaster profile was found for the associated address.');
        return;
      }
      const updated = { ...identity, farcasterFid: fid ?? identity.farcasterFid };
      await (await getStorage()).putIdentity(updated);
      useAuthStore.getState().setIdentity(updated);
      await persistRefreshedProfile(name, profile.pfp_url ?? undefined);
      setProfileRefreshFeedback(`Profile refreshed from @${profile.username || name}.`);
    } catch (error) {
      console.error('[Settings] Farcaster profile refresh failed:', error);
      setProfileRefreshFeedback('Could not refresh the Farcaster profile. Try again shortly.');
    } finally {
      setProfileRefreshSource(null);
    }
  };
            ...
            <div className="bg-primary-900/60 border border-primary-800/60 rounded-lg p-4 space-y-4 backdrop-blur">
              ...
              <div className="border-t border-primary-800/60 pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={handleRefreshFarcasterProfile}
                    disabled={!profileLookupAddress || profileRefreshSource !== null}
                    className="btn-secondary text-sm px-3 disabled:opacity-50"
                  >
                    {profileRefreshSource === 'farcaster' ? 'Refreshing Farcaster…' : 'Refresh from Farcaster'}
                  </button>
                </div>
