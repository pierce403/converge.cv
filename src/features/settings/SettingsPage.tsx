...
export function SettingsPage() {
  ...
  const handleReconnect = async (connectedWallet?: ConnectedWalletSnapshot) => {
    ...
    try {
      ...
      let effectiveWallet = walletSnapshot?.address.toLowerCase() ?? walletAddress?.toLowerCase();
      try {
        if (
          !walletSnapshot &&
          (!isWalletConnected || effectiveWallet !== identity.address.toLowerCase())
        ) {
          ...
          effectiveWallet = connectedAddress.toLowerCase();
        }
      } catch (connectErr) {
  ...
  const handleEnablePush = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(
          'Experimental push subscription registered. Live delivery and first-contact coverage are not yet verified.'
        );
      } else {
  ...
  const resolveWalletInboxTarget = async (
    ...
    if (basePublicClient) {
      for (const account of normalized) {
        try {
          const bytecode = await basePublicClient.getBytecode({ address: account });
          successfulInspections += 1;
          if (classifyWalletBytecode(bytecode) === 'SCW') {
            return { address: account, walletType: 'SCW', chainId: BASE_CHAIN_ID };
          }
        } catch (error) {
          console.warn('[Settings] Could not inspect wallet bytecode on Base:', error);
        }
      }
    }
  ...
  const connectExistingWalletInbox = async (
    ...
    console.info('[Settings] Added a fresh device key to existing inbox', {
            ...
            <div className="bg-primary-900/60 border border-primary-800/60 rounded-lg divide-y divide-primary-800/60 backdrop-blur">
              ...
              >
                <div>
                  ...
                  <div className="text-sm text-primary-200">Create a fresh browser key approved by a wallet that controls the inbox</div>
                </div>
              ...
              >
                <div>
                  ...
                  <div className="text-sm text-primary-200">
                    {canDownloadKeyfile
                      ? 'Plaintext private-key or recovery-phrase backup'
                  ...
                  <div>
                    ...
                    <div className="text-sm text-primary-200">
                      {pushStatus === 'unsupported'
                        ...
                        : pushStatus === 'enabled'
                          ? 'Experimental subscription registered; live delivery is unverified'
...
function ConnectExistingInboxModal({
  ...
  const runConnect = async (getAccounts: () => Promise<readonly string[] | undefined>) => {
    ...
    try {
      ...
      await onConnect(accounts, connectedSigner, setConnectionStep);
      onClose();
    } catch (err) {
  ...
  const runWalletConnection = async (option: (typeof inboxWalletOptions)[number]) => {
    ...
    try {
      ...
      setLastAccounts(accounts);
      setWalletSnapshot({ accounts, signMessage: result.signMessage });
      setConnectionStep(null);
    } catch (error) {
      ...
      <div className="w-full max-w-lg rounded-xl border border-primary-800/60 bg-primary-950 shadow-2xl">
        ...
        <div className="p-4 space-y-3">
          <p className="text-sm text-primary-200">
            Converge generates a fresh local key, registers or reuses one browser installation in the target inbox, and keeps your current inbox unchanged. It will not move a registered key; reassignment would strand that key&apos;s previous inbox.
          </p>
