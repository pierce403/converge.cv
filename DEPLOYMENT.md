...
- Ensure `src/lib/xmtp/client.ts` targets XMTP protocol v3 (currently pinned by the lockfile to `@xmtp/browser-sdk` 6.1.2)
...
The current minimal service worker handles push notification reception and the
cross-origin-isolation navigation response. Offline app-shell/static-asset
caching remains disabled while XMTP stability work continues.
...
- [ ] Confirm local storage warnings accurately describe unencrypted browser data
...
- XMTP message payloads are encrypted client-side before network transport.
- Converge currently stores local private keys, mnemonics, decrypted app data,
  attachment caches, and the Browser SDK SQLite database without encryption at rest.
- Keyfile exports contain plaintext recovery material.
- Incomplete passphrase, passkey, and vault-lock controls are not exposed.
- Push registration sends subscription/inbox/installation metadata and locally
  available conversation HMAC material to the configured relay; review the relay
